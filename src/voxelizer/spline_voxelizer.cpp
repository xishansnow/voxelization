...
std::unique_ptr<VoxelizerBase> SplineVoxelizer::create(
    ...
    if (spline_type < 0 || spline_type > 2) {
        throw std::invalid_argument("Invalid spline type");
    }

    if (use_gpu) {
        return std::make_unique<SplineVoxelizerGPU>(control_points, radius, spline_type);
    } else {
        return std::make_unique<SplineVoxelizerCPU>(control_points, radius, spline_type);
    }
}
...
void SplineVoxelizerCPU::voxelize_cpu(VoxelGrid& grid) {
    // Get grid dimensions and resolution
    const auto& dims = grid.get_dimensions();
    const auto& resolution = grid.get_resolution();
    const auto& min_bounds = grid.get_min_bounds();
    const auto& max_bounds = grid.get_max_bounds();
...
void SplineVoxelizerGPU::voxelize_gpu(VoxelGrid& grid) {
